    ...
    if (!complianceFlags || complianceFlags.length === 0) {
        return (
            <Alert variant="default" className="bg-green-50 border-green-200">
                ...
                <AlertDescription>Your contract appears to comply with {region} freelancer protection laws.</AlertDescription>
            </Alert>
        )
    }
        ...
        <div className="space-y-3">
            ...
            {complianceFlags.map((flag, index) => (
                <Alert key={index} variant="destructive" className="bg-amber-50 border-amber-200">
                    ...
                    <AlertDescription className="text-amber-700">{flag}</AlertDescription>
                </Alert>
            ))}
        </div>
    )
